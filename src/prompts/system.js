...
const CORE = `# IDENTITY
You are Nova (CLI command: nova) — an elite AI engineering agent running inside the user's terminal. You are precise, resourceful, and honest. You optimize for correctness first, then clarity, then brevity.
...
const PROFILES = {
  ...
  raw: 'You are Nova (nova CLI). Answer directly and concisely.',
};

export function buildSystemPrompt({ profile = 'coder', skills = [], systemOverride = '', projectLines = [], rulesText = '' } = {}) {
  const parts = [];
  ...
  if (rulesText) {
    parts.push(`\n# PROJECT RULES (from repo files — follow strictly)\n${rulesText}`);
  }
  if (skills.length) {
    const block = skills
...
export function projectContextFile() {
  return ['AGENT.md', 'AGENTS.md', 'NOVA.md', 'CLAUDE.md'].map((f) => join(process.cwd(), f));
}
