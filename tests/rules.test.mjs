...
const WS = '/tmp/kayno-rules-ws';
rmSync(WS, { recursive: true, force: true });
mkdirSync(join(WS, '.mij', 'skills'), { recursive: true });
mkdirSync(join(WS, '.github'), { recursive: true });

process.env.NOVA_HOME = '/tmp/kayno-rules-home';
...
console.log('priority order');
{
  ...
  writeFileSync(join(WS, '.mij', 'skills', 'a.md'), '- skill rule A');
  writeFileSync(join(WS, '.mij', 'skills', 'b.md'), '- skill rule B');
  ...
  ok(
    rules.filter((r) => r.source.startsWith('.mij/skills')).every((r) => r.priority < 90),
...
console.log('missing files + empty dir');
{
  ...
  rmSync(join(WS, '.mij', 'skills'), { recursive: true, force: true });
...
console.log('budget truncation');
{
  ...
  writeFileSync(join(WS, '.mij', 'skills', 'big.md'), 'y'.repeat(5000));
